...
function Complex2() {
  ...
  const generateRandomNumbers = () => {
    ...
    while (newQuestions.length < 30) {
      ...
      do {
        num1 = Math.floor(Math.random() * 101) + 1;
        num2 = Math.floor(Math.random() * 101) + 1;
      } while (num1 === num2 || num1 === 0 || num2 === 1|| num1 === 1);
...
// Function to check user's answer for all questions
  const checkAllAnswers = () => {
    ...
    const correct = [];
    const incorrect = []; // Define 'incorrect' array here
  ...
  // Only show the result popup if there are incorrect answers
...
return (
  <div className="addition-page">
    ...
    <Timer /> {/* Pass isPaused as a prop to Timer */}
